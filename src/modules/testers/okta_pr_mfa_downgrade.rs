        ...
        if !config.contains_key("OKTA_TEST_USER") || !config.contains_key("OKTA_TEST_PASSWORD") {
            ...
            return Ok(vec![Evidence {
                ...
                findings: vec![Finding {
                    ...
                    description: "Set OKTA_TEST_USER and OKTA_TEST_PASSWORD to an active Okta user enrolled in phishing-resistant MFA (WebAuthn) to enable this test.".to_string(),
                    severity_id: 0,
                }],
...
mod tests {
    ...
    fn missing_test_user_returns_unknown() {
        ...
        let ev = &PrMfaDowngradeTester.test(&config).unwrap()[0];
    ...
    fn mfa_challenge_status_with_pr_only_is_effective() {
        ...
        let body: serde_json::Value =
            serde_json::from_str(MFA_REQUIRED_WITH_WEBAUTHN_ONLY).unwrap();
    ...
    fn other_status_no_factors_is_effective_no_downgrade() {
        ...
        assert!(ev
            .findings
            .iter()
            .any(|f| f.title == "Only Phishing-Resistant Factors Offered"));
    }
    ...
    fn non_200_unknown_status_hits_else_branch() {
        ...
        assert!(ev
            .findings
            .iter()
            .any(|f| f.title == "No Downgrade Path Detected"));
    }
    ...
    fn mfa_enroll_response_details() {
        ...
        assert!(ev
            .findings
            .iter()
            .any(|f| f.title == "MFA Enrollment Incomplete"));
