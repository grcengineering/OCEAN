...
mod tests {
    ...
    fn metadata_complete() {
        ...
        let obs = ThreatInsightObserver;
        assert_eq!(obs.id(), "okta.threat_insight");
    ...
    #[test]
    fn excessive_zone_exclusions_adds_finding() {
        // More than 2 excluded zones adds an additional finding even when blocking
        let body = r#"{"action":"block","excludeZones":["z1","z2","z3"]}"#;
        let url = mock_server(200, body);
        ...
        assert!(
            ...
            "expected excessive-exclusions finding"
        );
    }

    #[test]
    fn audit_mode_with_excessive_exclusions_has_two_findings() {
        // audit mode + >2 exclusions → two findings
        let body = r#"{"action":"audit","excludeZones":["z1","z2","z3","z4"]}"#;
        let url = mock_server(200, body);
        let ev = &ThreatInsightObserver.observe(&base_config(&url)).unwrap()[0];
